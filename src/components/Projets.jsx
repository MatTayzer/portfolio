import Pikuit from "../assets/pikuit.png"
import Budget from "../assets/budget.png"

import { FaGooglePlay } from "react-icons/fa"

const projets = [
    {name: "Pikuit", image:Pikuit, description:"Créez vos recettes et listes de courses facilement et rapidement.", lang:["Flutter", "Firebase", "sqflite"], other:[{main:"3+", sub:"ans"}, {main:"10+", sub:"Installs"}], href:"https://play.google.com/store/apps/details?id=fr.tayzerapps.pikuit&hl=fr"},
    {name: "Budget Mensuel", image: Budget, description: "Notez vos dépenses et vos revenus pour avoir un suivi de votre argent.", lang: ["Flutter", "Firebase"], other: [{ main: "3+", sub: "ans" }, { main: "2+", sub: "Installs" }], href: "https://play.google.com/store/apps/details?id=fr.tayzerapps.mescomptes&hl=fr" },
]

function Projets() {
    return (
            <section id="projets" className="relative z-20 bg-[#171717] text-white py-20 px-8">
                <svg
                ...
                <div className="flex flex-wrap sm:flex-row justify-center gap-6">
                    {projets.map((projet) => (
                        <div key={projet.name} className="flex flex-col max-w-sm px-6 py-6 gap-2 bg-[#171717] border border-white/10 rounded-2xl shadow-lg hover:border-red-400 hover:scale-105 duration-400 ">
                            ...
                            <h1 className="font-medium">{projet.name}</h1>
                            ...
                            <a href={projet.href} className="flex flex-row gap-6 items-center py-2 px-4 bg-black/20 border border-white/20 rounded-sm">
                                <FaGooglePlay className="text-red-400"></FaGooglePlay>
                                <span className="font-medium text-sm">Disponnible sur le Play Store</span>
                            </a>
                        </div>
                    ))}
